...
const getUserName = () =>{
    let userName = prompt(`Welcome to the Bingo Game. What's your name?`)
...
const randomNumber = () => {
    let randomNumberBingo = Math.round(Math.random() * 45 + 1);
...
const showScore = () => {
    ...
    users.forEach((player) => {
        console.log(`Position: ${rankingPosition} | Player: ${player.user} | Score: ${player.score}`)
        rankingPosition++;
  });
    console.log("***********************");
};
...
const numbersBombo = () => {
    let numberBingoBombo = randomNumber();
...
const changeCard = () => {
    let changeNumbersCard = confirm("Do you like these numbers? If you want other numbers press cancel.")
...
const scoreTable = () => {
    if (won) {
        console.log(
          `You can see your score in the following table.`
        );
        totalPoints();
        showScore();
      }
